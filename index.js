const animateWeather = (weather, weatherID) => {
...
const updateGreeting = (name, sunrise, sunset) => {
    const title = document.getElementById('title');
    title.innerHTML = `${name}'s Homepage`;

    let currentTime = new Date();
    let msg = '';
    

    if(currentTime.getTime() > sunrise && currentTime.getHours() < 12){
        msg += 'Good Morning';
    } else if(currentTime.getTime() < sunset){
        msg += 'Good Afternoon';
    } else {
        msg += 'Good Evening';
    }
...
const setBackground = (weather, sunrise, sunset) => {
...
const getWeather = () => {
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?${query}&appid=${config.API_KEY}&units=imperial`)
        ...
        .then((data) => {
            console.log(data);
            //updateGreeting(getLS('user'), data.sys.sunrise, data.sys.sunset);
            displayLocation(getLS('city'));
            let weather = data.weather[0].main;
            updateLS('weather', weather);
            //setInterval(function(){updateGreeting(getLS('user'), data.sys.sunrise, data.sys.sunset)}, 60000);
            ...
            setBackground(weather, data.sys.sunrise, data.sys.sunset, data.coord.lat, data.coord.lon);
            //setInterval(function(){setBackground(weather, data.sys.sunrise, data.sys.sunset)}, 60000); //change time to speed up for testing, intended value: 30000
            animateWeather(weather, data.weather[0].id); //data.weather[0].main
        })
        .catch(error => {
            console.error(error);
...
const inputs = [inputUser, inputCity, inputZip, inputCountry];
...
const updateUser = () => {
    inputs.forEach(el => updateLS(el.id, el.value));
    //updateGreeting();
    location.reload();
}
...
if( /*getLS('user') &&*/ getLS('city') && getLS('country')){showSettings()};
...
//add all country codes as options to select

//live mode vs sandbox mode
// sandbox mode pauses updating
// opens menu to change weather, sky (color vs gray), and time
// fast preview of sunrise and sunset
